...
export default async function Home() {
  ...
  const { surveyCount, responseCount } = await getStats(userId);
  const recentSurveys = await getRecentSurveys(userId);
    ...
    <div className="min-h-screen">
      ...
      <section className="container mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="hover:border-[#c9c1ed] transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-[#6b6b7b]">Completion Rate</CardTitle>
              <BarChart3 className="w-4 h-4 text-[#6b6b7b]" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-['Syne'] font-bold">—</div>
              <p className="text-xs text-[#6b6b7b] mt-1">No data yet</p>
            </CardContent>
          </Card>
        </div>
      </section>
